<div class="relative" x-data>
    @if ($type === 'textarea')
        <textarea name="{{ $name }}" id="{{ $name }}" placeholder="{{ $placeholder }}"
            class="w-full rounded-md border-0 bg-gray-800 py-2 px-3 text-sm text-gray-100 ring-1 ring-gray-700 placeholder:text-gray-500 focus:ring-2 focus:ring-blue-500 focus:outline-none transition">{{ $value }}</textarea>
    @else
        ...
        input.focus();">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="size-3.5 text-gray-500 hover:text-gray-300 transition-colors">
